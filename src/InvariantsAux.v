...
From TLC Require Import LibContainers.
...
Lemma map_gp_aux_type : forall f (p_ : SExpRec),
  Mem (type p_) all_storable_SExpTypes ->
  type (map_gp f p_) = type p_.
Proof. introv I. destruct p_; apply~ SExpType_restrict_all_storable_SExpTypes. Qed.
...
Lemma map_gp_aux_may_have_types : forall (S S' : state) p p_ f l p',
  ...
  may_have_types S' l p'.
Proof.
  introv E W Il (p_'&R&T). forwards E': read_write_SExp_eq W. tests: (p' = p).
  - rewrite E in R. inverts R. eexists. splits*. rewrite~ map_gp_aux_type. applys~ BagInIncl Il.
  - eexists. splits.
...
Lemma map_gp_aux_list_type : forall (S S' : state) p p_ f l_t l_car l_tag l,
  ...
  list_type S' l_t l_car l_tag l.
Proof.
  introv E W I1 I2 I3 L. forwards E': read_write_SExp_eq W.
  induction L using list_type_ind.
  - applys~ list_type_nil. applys~ map_gp_aux_may_have_types E W. solve_incl.
  - applys~ list_type_cons. tests: (p0 = p).
    + rewrite H in E. inverts E. eexists. splits*.
      ...
      * simpl. rewrite~ SExpType_restrict_all_storable_SExpTypes.
        applys~ BagInIncl H1.
      * applys* map_gp_aux_may_have_types W.
      * applys* map_gp_aux_may_have_types W.
    + eexists. splits.
      * erewrite read_write_SExp_neq; autos*.
      * subst. do 4 eexists. splits*.
...
Lemma map_gp_aux_safe_pointer : forall (S S' : state) p p_ f p',
  read_SExp S p = Some p_ ->
  write_SExp S p (map_gp f p_) = Some S' ->
  safe_pointer S p' ->
  safe_pointer S' p'.
Proof.
  ...
  - (** safe_SExpRec_read **)
    ...
    + rewrite E' in R. inverts R.
      ...
      * (** SExpType_corresponds_to_datatype **)
        forwards OK: SExpType_corresponds_to_datatype OKp_. rewrite~ map_gp_aux_type.
        -- inverts~ OK; constructors~; try introv M;
             try applys~ map_gp_aux_may_have_types W;
             try applys~ map_gp_aux_list_type W; try solve_incl.
      ...
      * (** SExpRec_header **)
        ...
        -- (** safe_attrib **)
           ...
           applys~ IH OKp_.
        -- (** attrib_list **)
           applys~ map_gp_aux_list_type W; try solve_incl.
    ...
    + erewrite read_write_SExp_neq in R; autos*.
      ...
      * (** SExpRec_header **)
        ...
        -- (** attrib_list **)
           applys~ map_gp_aux_list_type W; try solve_incl. apply OKp_.
Qed.
...
Lemma map_gp_aux_list_type_safe : forall (S S' : state) p p_ f l_t l_car l_tag l,
  ...
  list_type_safe S' l_t l_car l_tag l.
Proof.
  introv E W I1 I2 I3 L. forwards E': read_write_SExp_eq W. induction L using list_type_ind.
  - applys~ list_type_nil. applys~ map_gp_aux_may_have_types W. solve_incl.
  - applys~ list_type_cons. tests: (p0 = p).
    + eexists. splits*. rewrite E in H. inverts H.
      ...
      * simpl. simpl in H1. rewrite~ SExpType_restrict_all_storable_SExpTypes.
        applys~ BagInIncl I1.
      * simpl. constructor.
        -- simpl. applys~ map_gp_aux_safe_pointer W. inverts~ H2.
        -- simpl. applys~ map_gp_aux_list_type W; try solve_incl. inverts~ H2.
      * applys* map_gp_aux_may_have_types W.
...
Proof.
  ...
  - constructor.
    ...
    + (** safe_SymbolTable **)
      ...
      applys~ map_gp_aux_list_type_safe W L; solve_incl.
  - constructor.
    + (** globals_not_NULL **)
      apply OKg.
    + (** globals_not_NULL_safe **)
      introv D'. applys~ map_gp_aux_safe_pointer W. applys~ globals_not_NULL_safe OKg.
    + (** R_NilValue_may_have_types **)
      applys~ map_gp_aux_may_have_types W; try solve_incl. applys~ R_NilValue_may_have_types OKg.
  Optimize Proof.
Qed.
