    ...
    let%success fargusedi :=
      ifb ftag <> R_DotsSymbol /\ ftag <> R_NilValue then
        ...
        as b, b_, b_list do
          ...
          else result_success fargusedi using S, runs, globals
      else result_success fargusedi in
    read%list _, a_cdr, _ := a in
    result_success (a_cdr, fargusedi :: fargusedrev) using S, runs, globals in
        ...
        ifb fargusedi = 0 then
          ifb f_tag = R_DotsSymbol /\ ~ seendots then
            ...
            as b, b_, b_list do
              ...
              else result_success fargusedi using S, runs, globals in
  ...
  whileb f <> R_NilValue /\ b <> R_NilValue /\ ~ seendots do
    ...
    ifb f_tag = R_DotsSymbol then
      ...
      ifb argused b_ <> 0 \/ b_tag <> R_NilValue then
        ...
        map%pointer b with set_argused 1 ltac:(nbits_ok) in
        result_success (f_cdr, a_cdr, b_cdr, seendots) using S, runs in
  result_skip.
...
Definition matchArgs_dots dots supplied :=
  ...
  as _, a_, _ do
    ifb argused a_ = 0 then
      ...
      result_success i using S, runs, globals in
  ifb i <> 0 then
    ...
    as _, b_, b_list do
      ...
      else result_success f using S, runs, globals in
    set%car dots := a in
    result_skip
  else result_skip.
...
Definition matchArgs_check supplied :=
  ...
  as _, b_, b_list do
    ...
    else result_success (unused, last) using S, runs, globals in
  ifb last <> R_NilValue then
...
Definition matchArgs formals supplied (call : SEXP) :=
  ...
  as _, _ do
    ...
    result_success (actuals, 1 + argi) using S, runs, globals in
