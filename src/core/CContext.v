  ...
  ifb context_cjmpbuf c = context_cjmpbuf cptr then
    ...
    run%success
      ifc (context_cloenv c '!= R_NilValue) '&& (context_conexit c '!= R_NilValue) then
        ...
        let%contextual NilValue := R_NilValue : _SEXP in
        let c := context_with_conexit c NilValue in
    ...
    run%success
      ...
      if match ExitContext with
         | None => false
         | Some ce => context_cjmpbuf ce '= context_cjmpbuf c
         end then
...
Definition endcontext cptr :=
  ...
  run%success
    ifc (context_cloenv cptr '!= R_NilValue) '&& (context_conexit cptr '!= R_NilValue) then
      let s := context_conexit cptr in
      read%state savecontext := R_ExitContext in
      let%contextual NilValue := R_NilValue : _SEXP in
      let cptr := context_with_conexit cptr NilValue in
  ...
  run%success
    ...
    if match ExitContext with
       | None => false
       | Some ce => context_cjmpbuf ce '= context_cjmpbuf cptr
       end then
      map%state update_R_ExitContext None in
      result_skip
    else result_skip in
